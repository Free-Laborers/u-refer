...
const main = async () => {
  ...
  const jobPost = await createJobPost({ hiringManagerId: manager.id, title: 'Software Engineer' })
  const jp2 = await createJobPost({ hiringManagerId: manager.id, title: 'Software Developer' })
  const jp3 = await createJobPost({ hiringManagerId: manager.id, title: 'CEO' })
  const jp4 = await createJobPost({ hiringManagerId: manager.id, title: 'HR Manager' })
