...
export default function JobFeed() {
  ...
  return (
    ...
    <Box height={'calc(100vh - 64px)'} sx={{ ml: `${drawerWidth}px` }}>
      <Box m={2} display='grid' height='100%' gridTemplateRows='auto 1fr auto' gridTemplateColumns='1fr 1fr' columnGap={2}>
        <Box my={1}>
          <Typography variant='button'>{'Sort by Date: '}</Typography>
          <IconButton onClick={() => handleChange()}>{getIcon(selectedSort)}</IconButton>
        ...
        <Pagination count={Math.floor(numPages)} page={page + 1} onChange={(e, page) => setPage(page - 1)} sx={{ mx: 'auto', my: 2 }} />
        <Box />
      </Box>
